...
main() {
  ...
  platform=$( (gen_list) | rofi --theme solarized -font 'hack 10' -dmenu -matching fuzzy -no-custom -location 0 -p "Search > " )

  if [[ -n "$platform" ]]; then
    query=$( (echo ) | rofi  --theme solarized -font 'hack 10' -dmenu -matching fuzzy -location 0 -p "Query > " )
